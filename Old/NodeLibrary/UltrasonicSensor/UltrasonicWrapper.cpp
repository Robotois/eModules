...
void UltrasonicWrapper::getValue(const FunctionCallbackInfo<Value>& args){
  Isolate* isolate = Isolate::GetCurrent();
  HandleScope scope(isolate);

  UltrasonicWrapper* temp_obj = ObjectWrap::Unwrap<UltrasonicWrapper>(args.Holder());

  args.GetReturnValue().Set(Number::New(isolate,temp_obj->ultrasonic->getValue()));
}

void UltrasonicWrapper::getBasicValue(const FunctionCallbackInfo<Value>& args){
  Isolate* isolate = Isolate::GetCurrent();
  HandleScope scope(isolate);

  UltrasonicWrapper* temp_obj = ObjectWrap::Unwrap<UltrasonicWrapper>(args.Holder());

  args.GetReturnValue().Set(Number::New(isolate,temp_obj->ultrasonic->getBasicValue()));
}
// void Temperature(){
